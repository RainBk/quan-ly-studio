...
function App() {
  ...
  const handleImage = async (e) => {
    ...
    if (file) {
      ...
      try {
        ...
        toast.success("Đã nén ảnh thành công!");
      } catch (error) {
        toast.error("Lỗi ảnh!");
      }
  ...
  const handleSave = async (e) => {
    ...
    const loadToast = toast.loading("Đang lưu dữ liệu...");
  ...
  // --- GIAO DIỆN ---
  return (
    <div className="flex h-screen bg-gray-50 font-sans overflow-hidden text-sm md:text-base">
      <Toaster position="top-center" reverseOrder={false} />
      
      {/* MENU NAVIGATION */}
      <div className="fixed bottom-0 w-full bg-white border-t flex justify-around p-2 z-50 md:static md:w-24 md:flex-col md:justify-start md:h-full md:pt-10 md:border-r safe-area-bottom shadow-lg md:shadow-none">
        {['products', 'bookings', 'staff', 'finance'].map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={`flex flex-col items-center gap-1 p-2 rounded-xl transition-all ${activeTab===tab ? 'bg-blue-50 text-blue-600 font-bold scale-105' : 'text-gray-400 hover:bg-gray-50'}`}>
            {tab==='products' && <Shirt size={22}/>} 
            {tab==='bookings' && <Calendar size={22}/>} 
            {tab==='staff' && <Users size={22}/>} 
            {tab==='finance' && <BarChart3 size={22}/>}
      ...
      <div className="flex-1 flex flex-col h-full pb-20 md:pb-0">
        <div className="bg-white p-4 shadow-sm flex justify-between items-center z-10 pt-safe-top sticky top-0">
          <div className="flex items-center gap-2">
            <div className="bg-blue-600 w-8 h-8 rounded-lg flex items-center justify-center text-white font-bold">S</div>
            <h1 className="font-bold text-lg uppercase text-gray-800 tracking-tight">Studio Pro</h1>
          </div>
          <button onClick={() => {setShowModal(true); setModalType(activeTab); setSelectedItem({})}} className="bg-blue-600 text-white px-3 py-2 rounded-xl font-bold flex items-center gap-2 shadow-lg hover:bg-blue-700 active:scale-95 transition">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
              ...
              {products.map(p => (
                <div key={p.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden group">
                  <div className="h-40 bg-gray-100 relative overflow-hidden">
                    {p.image ? <img src={p.image} className="w-full h-full object-cover transition-transform group-hover:scale-110"/> : <div className="flex items-center justify-center h-full text-gray-300"><ImageIcon/></div>}
                    <button onClick={()=>{setSelectedItem(p); setModalType('qr'); setShowModal(true)}} className="absolute bottom-2 right-2 bg-white/90 p-1.5 rounded-lg shadow backdrop-blur-sm active:scale-90 transition"><QrCode size={18}/></button>
                  </div>
                  <div className="p-3">
                    ...
                    <div className="flex justify-between mt-2 text-xs items-center">
                        ...
                        <span className={`px-2 py-1 rounded-full text-[10px] font-bold ${p.status==='available'?'bg-green-100 text-green-700':'bg-orange-100 text-orange-700'}`}>
          ...
          {activeTab === 'bookings' && bookings.map(b => (
            <div key={b.id} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex justify-between items-center relative overflow-hidden">
               <div className={`absolute left-0 top-0 bottom-0 w-1.5 ${b.status==='pending'?'bg-orange-500':'bg-green-500'}`}></div>
               <div className="pl-3">
                 <h3 className="font-bold text-gray-800 text-base">{b.customer}</h3>
                 <p className="text-sm text-gray-500 flex items-center gap-1.5 mt-1"><Calendar size={14}/> {b.date} <span className="text-gray-300">|</span> {b.type}</p>
                 <p className="text-xs text-blue-500 mt-1 font-medium">{b.phone}</p>
               </div>
               <div className="text-right">
                 <p className="font-extrabold text-lg text-gray-800">{b.total?.toLocaleString()}đ</p>
                 <p className="text-xs text-gray-400 font-medium bg-gray-100 px-2 py-1 rounded-lg inline-block">Cọc: {b.deposit?.toLocaleString()}</p>
               </div>
            </div>
          ))}
          ...
          {activeTab === 'staff' && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {staffs.map(s => (
                <div key={s.id} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
                   <div className={`w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-xl ${s.role==='Photo'?'bg-purple-500':s.role==='Makeup'?'bg-pink-500':'bg-blue-500'}`}>
                      {s.name.charAt(0)}
                   </div>
                   <div>
                      <h3 className="font-bold text-gray-800">{s.name}</h3>
                      <p className="text-xs uppercase font-bold text-gray-400 tracking-wider mb-1">{s.role}</p>
          ...
          {activeTab === 'finance' && (
             <div className="space-y-4">
                <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-6 rounded-2xl shadow-lg text-white">
                    <h3 className="text-blue-100 uppercase font-bold text-xs tracking-wider mb-1">Tổng Doanh Thu Dự Kiến</h3>
                    <p className="text-4xl font-extrabold">{bookings.reduce((sum, b) => sum + (b.total||0), 0).toLocaleString()} <span className="text-lg opacity-70">VNĐ</span></p>
                </div>
                <div className="grid grid-cols-2 gap-4">
                   <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
                      <p className="text-gray-400 text-xs font-bold uppercase">Tổng Lịch Đặt</p>
                      <p className="text-2xl font-bold text-gray-800">{bookings.length}</p>
                   </div>
                   <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
                      <p className="text-gray-400 text-xs font-bold uppercase">Nhân Sự</p>
        ...
        <div className="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-4 backdrop-blur-sm">
           {modalType === 'qr' && selectedItem ? (
               <div className="bg-white p-8 rounded-3xl w-80 text-center animate-bounce-in shadow-2xl">
                  ...
                  <button onClick={()=>window.print()} className="w-full bg-blue-600 text-white py-3 rounded-xl mt-6 flex justify-center gap-2 font-bold shadow-lg hover:bg-blue-700"><Printer size={20}/> IN TEM NGAY</button>
                  <button onClick={()=>setShowModal(false)} className="mt-4 text-gray-400 text-sm font-medium hover:text-gray-600">Đóng lại</button>
               </div>
           ) : (
               <div className="bg-white p-6 rounded-3xl w-full max-w-md animate-bounce-in relative max-h-[90vh] overflow-y-auto shadow-2xl">
                  <div className="flex justify-between items-center mb-6">
                    <h2 className="font-extrabold text-xl text-gray-800 uppercase tracking-tight">
                        {modalType==='products'?'Nhập Kho':modalType==='bookings'?'Lên Lịch':'Thêm Nhân Sự'}
                    </h2>
                  ...
                  <form onSubmit={handleSave} className="space-y-4">
                     {modalType === 'products' && <>
                        <div className="h-40 border-2 border-dashed border-blue-200 rounded-2xl flex justify-center items-center relative bg-blue-50 group hover:bg-blue-100 transition">
                           {isUploading ? (
                              <div className="text-blue-600 font-bold animate-pulse text-sm">Đang tối ưu ảnh...</div>
                           ) : selectedItem?.image ? (
                     ...
                     {modalType === 'bookings' && <>
                        ...
                        <input name="phone" placeholder="Số điện thoại liên hệ" className="w-full bg-gray-50 border-none p-4 rounded-xl font-medium"/>
